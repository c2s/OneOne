...
<script type="text/javascript" src="{{ asset('static/js/plugins/jquery.nestable.js') }}"></script>
  ...
  <div class="sbox-content"> 
	...
	<ul class="nav nav-tabs" style="margin:10px 0;">
		<li @if($active == 'top') class="active" @endif ><a href="{{ URL::to('admin/menu?pos=top')}}"><i class="icon-paragraph-justify2"></i> {{ Lang::get('core.tab_topmenu') }} </a></li>
		<li @if($active == 'sidebar') class="active" @endif><a href="{{ URL::to('admin/menu?pos=sidebar')}}"><i class="icon-paragraph-justify2"></i> {{ Lang::get('core.tab_sidemenu') }}</a></li>
	</ul> 
  ...
  <p> {{ Lang::get('core.t_tipsdrag') }}</p>	
			...
			@foreach ($menus as $menu)
				  <li data-id="{{$menu['menu_id']}}" class="dd-item dd3-item">
					<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu['menu_name']}}
						...
						<a href="{{ URL::to('admin/menu/index/'.$menu['menu_id'].'?pos='.$active)}}"><i class="fa fa-edit"></i></a></span>
					</div>
							...
							@foreach ($menu['childs'] as $menu2)
							 <li data-id="{{$menu2['menu_id']}}" class="dd-item dd3-item">
								<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu2['menu_name']}}
									<span class="pull-right">
									<a href="{{ URL::to('admin/menu/index/'.$menu2['menu_id'].'?pos='.$active)}}"><i class="fa fa-edit"></i></a></span>
								</div>
									...
									@foreach($menu2['childs'] as $menu3)
									 	<li data-id="{{$menu3['menu_id']}}" class="dd-item dd3-item">
											<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{ $menu3['menu_name'] }}
												<span class="pull-right">
												<a href="{{ URL::to('admin/menu/index/'.$menu3['menu_id'].'?pos='.$active)}}"><i class="fa fa-edit"></i></a>
												</span>
											</div>
										</li>	
									@endforeach
		 ...
		 {!! Form::open(array('url'=>'admin/menu/saveorder/', 'class'=>'form-horizontal','files' => true)) !!}
 ...
 <p> {{ Lang::get('core.t_tipsnote') }}	</p>
		...
		<div class="col-sm-7">
		 ...
		 {!! Form::open(array('url'=>'admin/menu/save/', 'class'=>'form-horizontal','files' => true)) !!}
			  ...
			  <div class="form-group">
				...
				@if($row['menu_id'] !='')
					<button type="button"onclick="SximoConfirmDelete('{{ url('admin/menu/destroy/'.$row['menu_id'].'?pos='.$active)}}')" class="btn btn-danger ">  Delete </button>
				@endif	
				</div>	  
