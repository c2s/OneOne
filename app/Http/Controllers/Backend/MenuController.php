...
class MenuController extends Controller {
	...
	public function getIndex( Request $request ,$id = null  )
	{
		...
		$this->data['modules'] 		= \DB::table('module')->where('module_type','!=','core')->get();
			...
			{			
				if(isset($m['children']))
				{
					...
					foreach($m['children'] as $l)					
					{
						...
						\DB::table('menu')->where('menu_id','=', $l['id'])
							->update(array('parent_id'=> $m['id'],'ordering'=>$b));	
						$b++;
					}							
				}
				\DB::table('menu')->where('menu_id','=', $m['id'])
	...
	function postSave( Request $request, $id =0)
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('admin/menu?pos='.$pos)
				->with('messagetext', 'Data Has Been Save Successfull')->with('msgstatus','success');
	...
	public function getDestroy(Request $request,$id)
	{
		...
		$menus = \DB::table('menu')->where('parent_id','=',$id)->get();
		...
		return Redirect::to('admin/menu?pos='.$request->input('pos'))
